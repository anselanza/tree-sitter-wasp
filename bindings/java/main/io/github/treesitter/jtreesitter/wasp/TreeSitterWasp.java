package io.github.treesitter.jtreesitter.wasp;

import java.lang.foreign.*;

public final class TreeSitterWasp {
    ...
    private static final TreeSitterWasp INSTANCE = new TreeSitterWasp();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterWasp() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_wasp");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-wasp");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-wasp");
